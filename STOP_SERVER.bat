...
echo ========================================
echo.
echo [INFO] Stopping server on port 5001...

powershell -NoProfile -ExecutionPolicy Bypass -Command "$connections=@(Get-NetTCPConnection -LocalPort 5001 -State Listen -ErrorAction SilentlyContinue); $listenerPid=@(); foreach ($connection in $connections) { if ($connection.OwningProcess -and -not ($listenerPid -contains $connection.OwningProcess)) { $listenerPid += $connection.OwningProcess } }; $launcherPid=@(); $serverPid=@(); foreach ($proc in @(Get-CimInstance Win32_Process)) { if ($proc.Name -eq 'cmd.exe' -and $proc.CommandLine -match 'START_SERVER_PERSISTENT\.bat.*--child' -and -not ($launcherPid -contains $proc.ProcessId)) { $launcherPid += $proc.ProcessId }; if ($proc.Name -in @('python.exe','pythonw.exe') -and $proc.CommandLine -match 'server\.py' -and -not ($serverPid -contains $proc.ProcessId)) { $serverPid += $proc.ProcessId } }; $allPids=@($listenerPid + $launcherPid + $serverPid | Select-Object -Unique); if (-not $allPids) { Write-Host '[INFO] No running dashboard server was found.'; exit 0 }; foreach ($procId in $allPids) { Write-Host ('[INFO] Stopping PID ' + $procId); Stop-Process -Id $procId -Force }"
